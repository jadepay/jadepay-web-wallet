$(document).ready(function() {
		...
		if(email.match(/[\s\w\d]+@[\s\w\d]+/g)){
			if($("#openPass").val().length>=10){
				if($("#openPass").val()==$("#openPassConfirm").val()){
					...
					qrcode.makeCode("jadepay:"+keys.address);
	...
	$("#walletConfirmSend").click(function(){
		...
		tx.addUnspent($("#walletAddress").html(), function(data){
			...
			if(dvalue>=total){
				...
				$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("You have a confirmed balance of "+dvalue+" JADE unable to send "+total+" JADE").fadeOut().fadeIn();
				thisbtn.attr('disabled',false);
			}
	...
	function walletBalance(){
		...
		coinjs.addressBalance($("#walletAddress").html(),function(data){
			if($(data).find("result").text()==1){
				...
				$("#walletBalance").html(v+" JADE").attr('rel',v).fadeOut().fadeIn();
			} else {
				$("#walletBalance").html("0.00 JADE").attr('rel',v).fadeOut().fadeIn();
			}
	...
	function scannerStart(){
		...
		if(navigator.getUserMedia){
			...
			QCodeDecoder().decodeFromCamera(document.getElementById('videoReader'), function(er,data){
				if(!er){
					var match = data.match(/^jadepay\:([13][a-z0-9]{26,33})/i);
	...
	$(".qrcodeBtn").click(function(){
		...
		if(ta.length>0){
			...
			qrstr = "jadepay:"+$('.address',thisbtn).val();
		}
